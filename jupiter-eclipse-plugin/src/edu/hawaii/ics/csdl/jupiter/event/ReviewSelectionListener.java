...
public class ReviewSelectionListener implements ISelectionListener {
	...
	/**
	 * Notified when selection is changed. Brings the opened code review editor
	 * to top if a code review file is selected and the editor associated with
	 * the file is opened.
	 * 
	 * @param workbenchPart
	...
	public void selectionChanged(IWorkbenchPart workbenchPart,
		...
		IEditorPart editorPart = workbenchPart.getSite().getPage()
				.getActiveEditor();
		if (editorPart != null
				&& editorPart.getEditorInput() instanceof IFileEditorInput) {
			IFile activeFile = ((IFileEditorInput) editorPart.getEditorInput())
					.getFile();
