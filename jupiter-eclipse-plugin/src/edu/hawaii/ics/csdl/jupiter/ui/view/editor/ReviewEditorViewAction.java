...
//        ReviewPluginImpl.getInstance().notifyListeners(type, kind);
      ...
      public IMenuCreator getMenuCreator() {
        return new IMenuCreator() {
          private UndoReviewIssueManager undoReviewIssueManager;
		...
		/** Ignore to implement this method. */
          ...
          public Menu getMenu(Control parent) {
            return undoReviewIssueManager.createPulldownMenu(new Menu(parent));
          }
    ...
    UNDO.setImageDescriptor(ReviewPluginImpl.createImageDescriptor("icons/undo.gif"));
    ...
    GOTO.setImageDescriptor(ReviewPluginImpl.createImageDescriptor("icons/goto.gif"));
    NEXT = new Action() {
      public void run() {
    ...
    NEXT.setImageDescriptor(ReviewPluginImpl.createImageDescriptor("icons/down.gif"));

    PREVIOUS = new Action() {
      public void run() {
        // saves the current issue.
        processSave();
        ReviewTableView view = ReviewTableView.getActiveView();
        Table table = view.getTable();
        int index = table.getSelectionIndex();
    ...
    PREVIOUS.setImageDescriptor(ReviewPluginImpl.createImageDescriptor("icons/up.gif"));

    SAVE = new Action() {
      public void run() {
        log.debug("save was called.");
        processSave();
        // bring issue view to top.
//        ReviewTableView.bringViewToTop();
        int type = ReviewEvent.TYPE_COMMAND;
    ...
    SAVE.setImageDescriptor(ReviewPluginImpl.createImageDescriptor("icons/save.gif"));
//    SAVE.setActionDefinitionId("edu.hawaii.ics.csdl.jupiter.action.save");
    ...
    CLEAR.setImageDescriptor(ReviewPluginImpl.createImageDescriptor("icons/clear.gif"));
  }
   ...
   */
  private static void validateCodeReview(ReviewIssue codeReview, String activeTabNameKey)
